...
export async function generateConsolidatedInvoice(
  ...
  try {
    const result = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      // Generate invoice number without collision (safe inside transaction)
      ...
      await tx.dailySalesLog.updateMany({
    ...
    return { success: true, message: `Facture consolidée ${result.invoiceNumber} créée avec succès`, invoiceId: result.invoice.id, invoiceNumber: result.invoiceNumber }
  } catch (error) {
